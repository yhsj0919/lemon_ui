...
1. MIUIX 的手机规格是 Lemon UI 手机方案的第三方候选值；适用的官方资料、目标设备截图与源码冲突时，按[设计基准](hyperos-design-baseline.md)复核设备、版本、状态和单位，再更新本文。
...
8. MIUIX 原始奇数尺寸保留在参考表中；有同状态证据时允许直接采用，差异要记录原因。基础间距优先使用项目网格，不按奇偶性机械改写字体或组件规格；参见[设计基准](hyperos-design-baseline.md)。
...
| BasicComponent | 最小高度 56，内容内边距 16，首部/正文/尾部间距 8；内容自然增高 |
| Checkbox | 视觉尺寸 26×26 |
...
MIUIX 源码候选规格是最小 58×40、圆角 16、水平内边距 16、垂直内边距 13、按钮字号
17；Hyper 手机主题目前选择最小 58×48、水平内边距 16、垂直内边距 8。普通、主要、禁用状态分别提供容器色和内容色。全局 Miuix indication 使用
...
| --- | --- | --- | ---: | --- | ---: | ---: | ---: | ---: |
| 手机 | 58×48 | 48×48 | 16 | 水平16、垂直8 | 16 | 24 | 8 | 18 |
